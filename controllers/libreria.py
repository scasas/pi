@auth.requires(auth.has_membership('admin') or \
               auth.has_membership('libreria'))
def articulos():
    titulo = 'Articulos'
    response.view = 'load.html'
    # ABM & Consulta 
    grid = SQLFORM.grid(Articulos_lib
        , csv=False
...
def egresos_new():
    ...
    response.view = 'saa/egresos_new.html'

    art = int(request.vars.articulo_id)
    articulo = Articulos_lib(art)

    query = 'SELECT articulo_id, nombre, sum(cantidad) FROM libreria_articulos where articulo_id=%s group by articulo_id' %art
...
def ingresos():
    ...
    response.view = 'load.html'
    # ABM & Consulta 
    grid = SQLFORM.grid(Ingresos_lib
        , csv=False
        , showbuttontext=False
        , maxtextlength=30
        , paginate=200
        , orderby = Ingresos_lib.articulo_id
    )
    return dict(form = grid, title=titulo)
...
def stock():
    ...
    query = 'SELECT articulo_id, nombre, sum(cantidad) FROM pi.libreria_articulos group by articulo_id'

    grid = db.executesql(query)

    # if 'pdf' in request :
    if 'pdf' in request.args:
        # response.view = 'report/libreria.html'
